...
const QuestionCard: React.FC<QuestionCardProps> = ({
  ...
  return (
    <motion.div 
      className="bg-gray-900/80 backdrop-blur-sm rounded-xl shadow-lg overflow-hidden max-w-2xl w-full border border-purple-500/30"
      ...
      <div className="p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          {timeRemaining !== undefined && (
            <motion.div 
              className={cn(
                "flex items-center space-x-1 px-3 py-1 rounded-full border border-gray-700",
                getTimeColor()
              )}
              animate={{ scale: timeFlash ? 1.1 : 1 }}
          ...
          {question.options.map((option, index) => (
            ...
            >
              <Button
                ...
                className={cn(
                  ...
                  selectedAnswer === option 
                    ? "bg-gradient-to-r from-purple-700 to-purple-900 text-white border-purple-500" 
                    : "hover:bg-gray-800 hover:border-purple-500 text-purple-100 border-gray-700",
              ...
              >
                ...
                <span className="absolute inset-0 rounded-md bg-gradient-to-r from-purple-600/0 via-purple-600/40 to-purple-900/0 opacity-0 group-hover:opacity-100 transition-opacity" />
              </Button>
            </motion.div>
          ))}
        </div>
